...
class GameController:
    def __init__(self):
        self.mapping = {
            ...
            'Door': '🚪',
            'Trap': '💀',
        }  # словарь, через который преобразовываем имена объектов в значки.
        self.game_on = True  # маркер, сообщающий что пользователь не выразил желание прервать игру
        self.hero = Ghost  # инициализация игрового персонажа
        self.field = None  # игровое поле, с которым взаимодействуем

    def make_field(self):
        # метод, который будет отдавать по требованию готовое поле с размеченными значками элементами
        fields = []
    ...
    def play(self):
        # метод запускающий бесконечный цикл, который может быть прерван или по желанию игрока, или по прохождению
        # уровня
